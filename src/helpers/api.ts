...
export const requests = async (): Promise<IResponse> => {
  const response = await Axios.get("/requests");
  return response.data;
};

export const requestAccept = async (id: number): Promise<IResponse> => {
  const response = await Axios.patch("/requests/accept/" + id);
  return response.data;
};

export const requestDecline = async (id: number): Promise<IResponse> => {
